...
func main() {
	...
	// them globally via ADK telemetry.
	//
	// ADK reads this env var once, on first use, so it must be set before
	// telemetry.New. There is no option for it.
	...
	// 6. Create runner. AutoCreateSession makes Run create the session lazily
	// on first use, so the example doesn't manage the session lifecycle itself.
	...
	// 7. Run a multi-turn conversation. A fixed session ID makes both turns
